...
public class Client {
    ...
    private static Long connect(DatagramPacket packet, DatagramSocket dSocket) throws IOException {
     ...
     * @throws IOException
     */
    private void sendPacket(DatagramPacket packet, DatagramSocket dSocket) throws IOException {
        int retransmissions = 10;
        Message m = null;
        ...
        while(retransmissions > 0) {
            ...
            try {
                m = this.recQueue.poll(5, TimeUnit.SECONDS);
    ...
    private void sendMessage(String message) {
        new Sender(message, this).execute();
    }

    private class Sender extends SwingWorker<Void, Void> {
        String message;
        Client client;
        public Sender(String message, Client client) {
